...
def main():
    ...
    if args.config_name:
        config = AutoConfig.from_pretrained(args.config_name, use_flash_attention=False)
    elif args.model_name_or_path:
        config = AutoConfig.from_pretrained(args.model_name_or_path, use_flash_attention=False)
    else:
        raise ValueError(
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            # 梯度累积：只有在达到累积步数时才更新模型
            if (step + 1) % args.gradient_accumulation_steps == 0:
                # 梯度裁剪（如果不使用 DeepSpeed）
